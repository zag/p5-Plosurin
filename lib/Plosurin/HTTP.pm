...
sub RenderSoy {
    ...
    unless (ref($content)) { return $r->error404("Error while parse $fpath: $content")};
  ...
  #  return $f;
    no strict 'refs';
    return $call_name->();
    return "<pre>".Dumper($code,\@tmpls)."</pre>"
}
    ...
    while ( my $el = readdir DIR ) {
        ...
        if ( -d $fpath ) {
           ...
           $el .= qq!<p style="color: red;">$content</p>!;
              ...
              my ( $code, @tmpls ) = @$content;
              foreach my $t ( @tmpls ) {
                $el .= qq! &nbsp;&nbsp;<a href="/RenderSoy?path=$fpath&template=$t->{name}">$t->{name}</a>!;
              }
